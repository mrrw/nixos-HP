...
{
	#Display Help.
	echo "USAGE:  gamemaker [--options]; [user input]; [user input]"
	echo "DESCRIPTION:  Prompts lead to the creation of a simple game."
	echo
	echo "OPTIONS:"
...
{ #{{{  ----- App_RUN:  Initialization and termination
  SetLogs()
  { #{{{
    file_log_0=$HOME/var/log/gamemaker_0.log && fl0=$file_log_0
    file_log_1=$HOME/var/log/gamemaker_1.log && fl1=$file_log_1
    file_log_2=$HOME/var/log/gamemaker_2.log && fl2=$file_log_2
  # curate logs:
    [ -s $fl1 ] && mv $fl1 $fl2
    [ -s $fl0 ] && mv $fl0 $fl1
  # open main log:
    exec 3<> $fl0                    # Open log under file descriptor 3
s="Initializing gamemaker.sh..." && App_LOG #&& App_ECHO
  } #}}}
  SetOutput()
  { #{{{
...
dir_conf=$HOME/.config/gamemaker
dir_home=$HOME/gamemaker
dir_lib=$HOME/lib/gamemaker && lib=$dir_lib
dir_lib_hash=$lib/hash && lib_hash=$dir_lib_hash
dir_log=$HOME/var/log/gamemaker
dir_var=$HOME/var/gamemaker && var=$dir_var
dir_var_hash=$var/hash && var_hash=$dir_var_hash
file_conf_0=$HOME/.config/gamemaker/gamemaker.conf
file_log_0=$HOME/var/log/gamemaker.log
  # 
...
#
  ...
  Run()
  {
    ...
    if [ $daemon = "y" ] ; then
       s="Starting gamemaker as daemon; exiting to terminal." && App_ECHO && App_LOG
      Exec_gamemaker&  # adding & at the end of a command runs it in the background.
    else
       s="Executing gamemaker main program." && App_LOG #&& App_ECHO
      Exec_gamemaker   # Program will run in termimal until complete.
       s="Exiting gamemaker.bash." && App_LOG #&& App_ECHO
    fi
  }
...
Exec_gamemaker()
{ #{{{  ----- Exec_gamemaker:  execute main command
  #
  s="Executing main command." && App_LOG
  #
  # Main command:
  s="Echoed \"Welcome to bash gamemaker, by MRRW.\"." && App_LOG
	lib1=$lib/gametype.txt
  echo "Welcome to bash gamemaker, by MRRW."
	echo "What type of game do you want to make?"
	read r
	echo "$r" >> $lib1
