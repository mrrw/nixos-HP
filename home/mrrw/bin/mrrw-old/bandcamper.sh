...
{
	#Display Help.
	echo "USAGE:  bandcamper [--options]; [user input]; [user input]"
	echo "DESCRIPTION:  Process .zip files downloaded from bandcamp.com."
	echo
	echo "OPTIONS:"
...
{ #{{{  ----- App_ACTIVATE:  Initialization and termination
  ...
  #
file_log_0=$HOME/var/log/bandcamper_0.log && fl0=$file_log_0
file_log_1=$HOME/var/log/bandcamper_1.log && fl1=$file_log_1
file_log_2=$HOME/var/log/bandcamper_2.log && fl2=$file_log_2
  # curate logs:
[ -s $fl1 ] && mv $fl1 $fl2
[ -s $fl0 ] && mv $fl0 $fl1
  # open main log:
...
#exec 1>&4                        # OPTIONAL:  Send all stdout to tty 4
  ...
  ###  INITIALIZE
  #
s="Initializing bandcamper.sh..." && App_LOG && App_ECHO
  #
...
if [ $daemon = "y" ] ; then
	...
	s="Starting bandcamper as daemon; exiting to terminal." && App_ECHO && App_LOG
else
	App_START   # Program will run in termimal until complete.
	s="Exiting bandcamper.sh." && App_ECHO && App_LOG
fi
exit
...
dir_conf=$HOME/.config/bandcamper
dir_downloads=$HOME/usr/Downloads
dir_home=$HOME/bandcamper
dir_lib=$HOME/lib/bandcamper && lib=$dir_lib
dir_music=$HOME/usr/Music
dir_log=$HOME/var/log
dir_var=$HOME/var/bandcamper && var=$dir_var
file_conf_0=$HOME/.config/bandcamper/bandcamper.conf
file_log_0=$HOME/var/log/bandcamper.log
file_tmp=$var/.tmp && tmp=$file_tmp
...
{ #{{{  ----- Exec_ARMvars:  execute command
	...
	# Prep
	s=$(head -n 1 $tmp)
	sArt=$(echo $s | awk -F- '{ print $1 }' | sed 's/\ //g')
